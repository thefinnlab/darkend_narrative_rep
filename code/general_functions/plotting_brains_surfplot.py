...
__all__ = ['map_values_to_atlas', 'calc_alpha', 'make_translucent_map', 'Plot', 'check_surf', '_set_layout', '_flip_hemispheres', '_check_data', '_find_color_range']
...
class Plot(object):
    ...
    def add_layer(self, data, cmap='viridis', alpha=1, color_range=None,
                      ...
                      cbar_label=None, outline_thickness=1):
            ...
            # Apply the colormap and transparency settings
            for k, v in self.surfaces.items():
                if k in vertices.keys():
                    if as_outline:
                        # Get the labeling border and apply dilation for thicker contours
                        x = get_labeling_border(v, np.nan_to_num(vertices[k])).astype(float)

                        # Apply dilation to thicken the contour
                        for _ in range(outline_thickness - 1):  # Apply dilation multiple times
                            x = scipy.ndimage.binary_dilation(x).astype(float)
                    else:
                        x = vertices[k]
                    if zero_transparent:
                        x[x == 0] = np.nan
...
def map_values_to_atlas(atlas, values):
    """
    ...
    """
    # Convert atlas to an integer array
    atlas = np.asarray(atlas, dtype=int)
    values = np.asarray(values)

    # Initialize result with NaNs (for background)
    result = np.full(atlas.shape, np.nan)

    # Handle valid (non-background) values in atlas
    valid_mask = (atlas != 0) & ~np.isnan(atlas)  # Ensure we ignore 0 (background) and NaN

    # Shift the valid atlas labels by 1 for indexing (if necessary)
    atlas_shifted = atlas[valid_mask] - 1  # Assuming labels start at 1, shift by 1 for zero-indexing

    # Ensure the shifted labels are within bounds of the values array
    atlas_clipped = np.clip(atlas_shifted, 0, len(values) - 1)

    # Map the values to the corresponding valid regions
    result[valid_mask] = values[atlas_clipped]
...
"""
    if abs(VAL) > THR :
        ...
        return 2*abs(VAL)/THR
    elif np.isnan(VAL):
        return 0.0
    else:
        return 0.0

def make_translucent_map(zvals_data, threshold, beta_data):
    alpha = np.vectorize(calc_alpha)(zvals_data, threshold) # create a transparency/alpha mask
    beta_transl = np.multiply(beta_data, alpha) # weight betas by the alpha mask
    return beta_transl
